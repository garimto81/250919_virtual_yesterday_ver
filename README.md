...
### v3.6.0 (2025-09-22) - 빈 좌석 클릭으로 플레이어 빠른 추가 기능
- 🎯 **빈 좌석 클릭**: 빈 좌석 클릭 시 해당 좌석에 플레이어 즉시 추가 가능
- 🚀 **빠른 추가**: prompt로 플레이어 이름 입력 후 Google Sheets에 자동 저장
- 🔍 **중복 검사**: 같은 테이블 내 플레이어 중복 및 좌석 중복 자동 체크
- 🎨 **시각적 개선**: 빈 좌석에 + 아이콘과 hover 효과 추가
- ✅ **실시간 업데이트**: 추가 후 UI 즉시 새로고침으로 변경사항 반영
