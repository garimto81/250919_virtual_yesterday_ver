...
> **버전**: v3.5.26 (Apps Script v71.0.4)
> **최종 업데이트**: 2025-09-23
...
| **5차** | 브라우저 캐시 문제<br>CAM 로직 잔재 | 1. 버전 쿼리 파라미터 추가 (v3.5.25)<br>2. CAM 관련 코드 제거 (v3.5.26) | ✅ 해결 |
...
2. **CAM 관련 코드 제거 (v3.5.26)**
   - state.camNumbers 변수 제거
   - state.camPreset 변수 제거
   - CAM 버튼 이벤트 리스너 비활성화
   - Index 메타데이터에서 CAM 필드 기본값으로 설정
   - CAM 로깅 제거
